...
int next_worker(Env* env) {
    ...
    if (env->idle_workers > 0) {
        ...
        env->idle_workers--;
        if (env->idle_workers == 0) {
            MPI_Irecv(&env->results[env->next], 1, env->result_type,
                      MPI_ANY_SOURCE, TAG_DATA, MPI_COMM_WORLD, &env->recv);
        }
    } else {
        MPI_Status status;
        MPI_Wait(&env->recv, &status);
        int current = env->next;
        env->next = (env->next + 1) % 2;
        MPI_Irecv(&env->results[env->next], 1, env->result_type,
                  MPI_ANY_SOURCE, TAG_DATA, MPI_COMM_WORLD, &env->recv);
        p = status.MPI_SOURCE;
        Task task = env->worker_tasks[p - 1];
        result_t result = env->results[current];
    ...
    if (T->height < env->depth) {
        ...
        */
        ...
        for (int i = 0; i < n_moves; i++) {
            ...
            task->move = moves[i];
            task->parent = node;
            play_move(T, moves[i], &task->child);
            MPI_Isend(&task->child, 1, env->tree_type, p, TAG_TASK, MPI_COMM_WORLD, &env->send);
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    if (e.rank == 0) {
        ...
        MPI_Recv(&trees[current], 1, e.tree_type, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
        if (status.MPI_TAG != TAG_STOP) {
            MPI_Irecv(&trees[next], 1, e.tree_type, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &recv);
            evaluate(&trees[current], &results[current]);
            MPI_Isend(&results[current], 1, e.result_type, 0, TAG_DATA, MPI_COMM_WORLD, &send);
            while (1) {
                current = next;
                next = (current + 1) % 2;
